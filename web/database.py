...
def migrate_from_json():
        ...
        try:
            ...
            with get_db() as conn:
                ...
                for username, user in users_data.items():
                    try:
                        cursor.execute('''
                            INSERT OR IGNORE INTO users (username, password, salt, phone, created_at)
                            VALUES (?, ?, ?, ?, ?)
                        ''', (username, user['password'], user['salt'], user['phone'], user.get('created_at', datetime.now().isoformat())))
                        
                        # 迁移自选列表
                        for item in user.get('watchlist', []):
                            try:
                                cursor.execute('''
                                    INSERT OR IGNORE INTO watchlist (username, symbol, name, type, position, cost_price, added_at)
                                    VALUES (?, ?, ?, ?, ?, ?, ?)
                                ''', (username, item['symbol'], item.get('name'), item.get('type'), 
                                      item.get('position'), item.get('cost_price'), item.get('added_at', datetime.now().isoformat())))
                            except:
                                pass
                    except Exception as e:
    ...
    # 迁移报告数据
    reports_file = json_dir / "reports.json"
    if reports_file.exists():
        try:
            reports_data = json.loads(reports_file.read_text(encoding='utf-8'))
            with get_db() as conn:
                cursor = conn.cursor()
                for username, user_reports in reports_data.items():
                    for symbol, report in user_reports.items():
                        try:
                            cursor.execute('''
                                INSERT INTO reports (username, symbol, name, report_data, created_at)
                                VALUES (?, ?, ?, ?, ?)
                            ''', (username, symbol, report.get('name', symbol), 
                                  json.dumps(report.get('report_data', report), ensure_ascii=False),
