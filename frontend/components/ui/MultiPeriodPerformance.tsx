...
const defaultData: PeriodData[] = [
  { period: '1d', label: 'Âë®Êúü | Ê∂®Ë∑åÂπ?, return: -4.5 },
  { period: '5d', label: '5Êó?, return: -4.5 },
  { period: '10d', label: '10Êó?, return: -1.63 },
  { period: '20d', label: '20Êó?, return: -7.64 },
  { period: '60d', label: '60Êó?, return: 3.69 },
  { period: '120d', label: '120Êó?, return: 44.75 },
  { period: '250d', label: '250Êó?, return: 13.09 },
];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 mb-3 sm:mb-4 flex-wrap">
        ...
        <h3 className="text-xs sm:text-sm font-bold text-slate-300 tracking-wide">Â§öÂë®ÊúüË∂ãÂäøÂàÜÊû?/h3>
        <span className="hidden sm:inline text-xs text-slate-500 uppercase">MULTI-PERIOD TREND ANALYSIS</span>
      </div>

      {/* Performance Grid - ÂìçÂ∫îÂº?*/}
      ...
      {/* Footer Stats - ÂìçÂ∫îÂº?*/}
      <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-white/[0.05] grid grid-cols-2 gap-2 sm:gap-3">
        <div className="text-center">
          <div className="text-[9px] sm:text-[10px] text-slate-500 uppercase tracking-wider mb-1">ÊúÄ‰Ω≥Ë°®Áé?/div>
          <div className="text-xs sm:text-sm font-mono font-bold text-rose-400">
            +{Math.max(...data.map(d => d.return)).toFixed(2)}%
          </div>
        </div>
        <div className="text-center">
          <div className="text-[9px] sm:text-[10px] text-slate-500 uppercase tracking-wider mb-1">ÊúÄÂ∑ÆË°®Áé?/div>
